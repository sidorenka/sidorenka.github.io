...
;(function(window, document, undefined) {
  ...
  function loadAllImages(cb) {
    var index = 0, loaded = {}, loader = function() {
  ...
  var DogeText = function DogeText(cx, cy, text, angle, offset, shift, color) {
  ...
  HeartParticle.prototype.position = function(cx, cy) {
    ...
    this.oy = this.y = cy + 11 * (-(15 * Math.cos(this.i) - 5 * Math.cos(2 * this.i) - 2 * Math.cos(3 * this.i) - Math.cos(4 * this.i)));
  };
    ...
    loadAllImages(function(loadedAssets, errorMessage) {
      if(typeof errorMessage !== "string") {
        var c = new Canvas(),
            ...
            i,
            p = [],
            cds = scale(loadedAssets.doge.width, loadedAssets.doge.height, 160, 160); // center doge size scaled down
        ...
        p.push(new DogeText(cx, cy, "many love", 45, 50, -100, new RGB(255, 0, 255)));
        p.push(new DogeText(cx, cy, "such faces", -55, -220, -110, new RGB(255, 255, 0)));
        p.push(new DogeText(cx, cy, "many trippy", 55, 160, -160, new RGB(0, 255, 0)));
        p.push(new DogeText(cx, cy, "such dark", -12, 260, -20, new RGB(0, 100, 255)));
        p.push(new DogeText(cx, cy, "many heart", 50, -180, 110, new RGB(255, 100, 0)));
        p.push(new DogeText(cx, cy, "much doge", -50, 140, 140, new RGB(0, 255, 100)));
        ...
        (function anim() {
          ...
          ctx.drawImage(loadedAssets.doge, cx - (cds.w / 2) - 10, cy - (cds.h / 2), cds.w, cds.h);
        }());
      } else {
        alert(errorMessage);
      }
